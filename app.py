...
#preprocessing data
movies = pd.read_csv('movies.dat', sep='::', names=['MovieID', 'Title', 'Genres'], engine='python', encoding='latin')
...
movie_ids = S.index
sample_movie_ids = movie_ids[:100] #displaying 100 movies
...
for row_i in range(num_rows):
    ...
    for col, i in zip(cols, range(start_idx, end_idx)):
        ...
        image_id = mid[1:]  
        image_path = f"MovieImages/{image_id}.jpg"
...
if st.button("Get Recommendations"):
    ...
    for row_i in range(num_rows):
        ...
        for col, idx in zip(cols, range(start_idx, end_idx)):
            ...
            rec_image_path = f"MovieImages/{rec_image_id}.jpg"
